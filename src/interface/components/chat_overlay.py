...
class ChatOverlay(UIComponent):
    ...
    _just_opened: bool
    # NOTE: This is a callable function, you need to give it a function that sends the message
    _send_callback: Callable[[str], bool] | None
    # NOTE: This is a callable function, you need to give it a function that gets the messages
    ...
    def _handle_typing(self) -> None:
        shift = input_manager.key_down(
            pg.K_LSHIFT) or input_manager.key_down(pg.K_RSHIFT)
        ...
        if input_manager.key_pressed(pg.K_RETURN) or input_manager.key_pressed(pg.K_KP_ENTER):
            ...
            if txt and self._send_callback:
                ...
                try:
                    ...
                    ok = False
                    print(f"[ChatOverlay] send_callback raised exception: {e}")
                print(
                    f"[ChatOverlay] Attempted to send: '{txt}', Success: {ok}")
                if ok:
                    self._input_text = ""
                else:
                    print(
                        f"[ChatOverlay] Message not sent! Input still: '{self._input_text}'")
